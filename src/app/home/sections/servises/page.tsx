...
export default function ServisesSection() {
  ...
  return (
    <div className="pt-20 md:pt-20 px-6 md:px-60 min-h-[60vh] pb-10 md:pb-20 md:text-left text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-8">
        {services.map((service, index) => (
          <div key={index} className="group cursor-pointer">
            <div className="h-60 sm:h-80 md:h-96 lg:h-120 w-full mb-3 sm:mb-4 overflow-hidden rounded-xl md:rounded-2xl lg:rounded-3xl relative">
              <Image
                src={service.image}
                alt={service.title}
                fill
            ...
            <div className="flex justify-between items-start pb-8">
              <div>
                <h3 className="text-card-title mb-1 sm:mb-2">
                  <AnimatedText>{service.title}</AnimatedText>
                </h3>
                <p className="text-card-description">
                  <AnimatedText>{service.description}</AnimatedText></p>
              </div>
              <GoArrowUpRight className="text-2xl mt-1 transition-opacity duration-300" />
            </div>
          </div>
        ))}
